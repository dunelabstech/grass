...
def open_github(event):
    url = "https://github.com/stormwaves/grass"
    subprocess.Popen(['start', url], shell=True)


def open_referral(event):
    url = "https://app.getgrass.io/register/?referralCode="
    subprocess.Popen(['start', url], shell=True)
...
if __name__ == '__main__':
    ...
    github_label = tk.Label(labels_frame, text="GitHub репозиторий: https://github.com/stormwaves/grass    ",
                            fg="blue")
    github_label.pack(side=tk.LEFT)

    referral_label = tk.Label(labels_frame,
                              text="Реферальная ссылка: https://app.getgrass.io/register/?referralCode=HQGXP-wqOqOSTKu",
                              fg="red")
    ...
    labels_frame.pack()
    log_box.insert(tk.END, 'Инструкция:\n', 'info')
    log_box.insert(tk.END, '        1. Зарегистрируйтесь по нашей реферальной ссылке: https://app.getgrass.io/register/?referralCode=HQGXP-wqOqOSTKu\n', 'info')
    log_box.insert(tk.END, '        2. Введите свой userId\n', 'info')
    log_box.insert(tk.END, '        3. Укажите Socks5 Proxy в следующем формате: socks5://user:password@ip:port\n', 'info')
    log_box.insert(tk.END, '        4. Нажмите на кнопку Start\n', 'info')
